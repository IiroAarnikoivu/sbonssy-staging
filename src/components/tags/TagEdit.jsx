...
const TagEdit = () => {
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (result.isConfirmed) {
          const resp = await api.put(`/admin/tags/${id}`, values);
          if (resp) {
            Swal.fire({
              title: toastAlert("tagUpdatedSuccess"),
              position: "top-right",
              icon: "success",
              toast: true,
        ...
        setServerError("Failed to update tag. Please try again.");
      }
    },
  });
  ...
  useEffect(() => {
    (async () => {
      try {
        const resp = await api.get(`/admin/tags/${id}`);
        // Ensure we set full object to edit both locales
        const tag = resp.tag;
        formik.setValues({ name: tag?.name || { en: "", fi: "" } });
      } catch (err) {
        setServerError("Failed to load tag. Please try again later.");
      } finally {
        setLoading(false);
      }
    })();
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">{t("editTag")}</h1>
      {serverError && (
