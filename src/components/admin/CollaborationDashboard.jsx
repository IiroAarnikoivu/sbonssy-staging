...
export default function CollaborationDashboard() {
  ...
  const chartOptions = {
    title: `Collaboration Growth (${state.selectedYear})`,
    chartArea: { width: "80%", height: "70%" },
    hAxis: { title: "Month" },
    vAxis: { title: "Collaborations", minValue: 0 },
    ...
    (async () => {
      try {
        const resp = await api.get(
          `/admin/total-collaboration?year=${state.selectedYear}`
        );

        setState((prev) => ({
          ...prev,
          totalCollaborations: resp?.total || 0,
          collaborationsByMonth:
            resp?.data?.map((item) => ({
              ...item,
              year: resp?.year,
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow col-span-1 md:col-span-2 lg:col-span-3">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold">Collaboration Growth</h3>
            <select
