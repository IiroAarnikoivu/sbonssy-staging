...
import { processFullRefund, createAudit } from "@/lib/payments/refundUtils";
 ...
 * Dedicated server-to-server webhook endpoint for processing PPS order cancellations
 */
export async function POST(req) {
  ...
  try {
    ...
    // cancellations are always full removals
    const result = await processFullRefund(purchaseEvent, {
      reason: payload.reason || "Order cancelled via generic webhook",
      webhookId: webhookEventId,
      source: "Cancellation Webhook",
    });
    ...
    await createAudit(
      ...
      {
        source: "generic_cancellation_webhook",
        transactionId,
        payload,
