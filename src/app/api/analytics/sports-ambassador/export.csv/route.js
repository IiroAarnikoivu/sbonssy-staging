...
export async function GET(req) {
  try {
    ...
    // Aggregate per campaign for this ambassador similar to analytics route
    ...
    const rows = Array.from(map.values()).map((c) => {
      ...
      return {
        ...
        ambassadorEarnings: Number(c.ambassadorEarnings).toFixed(2),
        brandSpend: Number(c.brandSpend).toFixed(2),
      };
    });
    ...
    console.error("Export sports ambassador CSV error:", error);
    return NextResponse.json(
